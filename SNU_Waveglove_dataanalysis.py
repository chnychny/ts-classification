...
def analyze_feature_importance(x_data, y_data, feature_names=None):
    ...
    plt.tight_layout()
    plt.savefig(data_path+'/combined_figure/Top 20 Most Important Channels Average Time_steps frame '+str(seg_cut)+'.png')
    plt.close()
...
data_type = "SNUWG_selftouch"
condition = 'All_even'
seg_cut = 60

# 데이터 로드
data_path = "G:/My Drive/1. 연구/3. hand gesture recognition using IMU data/log_seq_250311/w_o object/"
...
df = metadata_df
df['unit'] = df['f_list']+'-'+df['label'].astype(str)+'-'+df['seg_idx'].astype(str)

# 데이터 분할 (f_list 기준)
train_mask = df['f_list'] != '250310_2_hr'
test_mask = df['f_list'] == '250310_2_hr'
...
#################################################tsne 시작
plot_tsne_by_channels(x_data, y_data, save_path=data_path+f'/combined_figure/Tsne frame_{seg_cut}')
# 각 센서 타입별로 TSNE 수행
for sensor_type, (start, end) in sensor_types.items():
    ...
    plot_tsne_by_channels(
        ...
        save_path=f"{data_path}/combined_figure/Tsne_{sensor_type}_frame_{seg_cut}"
    )
...
plt.tight_layout()
plt.savefig(f"{data_path}/combined_figure/Top 20 Most Important Channels frame {seg_cut}_new.png")
plt.close()
...
plt.tight_layout()
plt.savefig(f"{data_path}/combined_figure/Average Importance by Sensor Type frame {seg_cut}.png")
plt.close()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(f"{data_path}/combined_figure/Sum Importance by Sensor Type frame {seg_cut}.png")
plt.close()