...
from models.basictransformer import BasicTransformer2D
from models.projDilResTransformer2d import ImprovedProjDilResTransformer2_2d
...
if __name__ == "__main__":
    # suffix 설정
    suffix = f'ts_ws{common_params["window_size"]}_ov{common_params["overlap"]}'
    for exp_rep in range(common_params['exp_rep_time']):
        ...
        for exp_config in experiment_configs:
            ...
            try:
                ...
                # 실험할 모델들 정의
                models_to_train = [
                    #(ImprovedProjDilResTransformer2_2d(in_channels=30, output_channel=64, num_classes=num_classes, stride=2).to(device), 
                    #"ImprovedProjDilResTransformer2_2d"),  
                    #(projDil3Resnet3_2d(in_channels=30, output_channel=64, num_classes=num_classes, stride=2).to(device), 
                    #"projDil3Resnet3_2d"),       
                    (BasicTransformer2D(in_channels=30, output_channel=64, num_classes=num_classes, d_model=64, n_head=8, dim_feedforward=64, dropout=0.1, num_transformer_layers=3).to(device), 
                    "BasicTransformer2D"),                                                       
                ]
