...
def train_model(label_to_idx):
    ...
    for epoch in range(epochs):
        ...
        #     device, accumulation_steps, grad_step_update=True
        # )
...
# SNU waveglove
    ...
    epochs = 150
    lr = 1e-3
    exp_name = f'{data_type}-{condition}-{str(test_condition)}-noise{noise_level}-temp-epoch{epochs}-lr{lr}-bsize{batch_size}-{suffix}'
    # 데이터 로드
